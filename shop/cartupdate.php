...
//$top_seller = $dao -> getTopSeller($_POST['it_id'][0] , $_POST["marketing_member"]);
//if($top_seller["top_seller"] == $member["mb_id"]){
//    alert("입력하신 추천인은 이미 입력되어있습니다.");
//}

if(!$dao->checkMember($_POST["marketing_member"]) && $_POST["marketing_member"] != ""){
    alert("추천인을 다시 한번 확인 해 주세요. (등록된 회원이 아닙니다)");
}else if($_POST["marketing_member"] == $member["mb_id"] && empty($member["mb_id"]) == false){
    alert("추천인을 다시 한번 확인 해 주세요. (본인이 추천인이 될수없음 )");
}
...
else // 장바구니에 담기
{
    ...
    for($i=0; $i<$count; $i++) {
        ...
        $sql = " INSERT INTO {$g5['g5_shop_cart_table']}
                        ( od_id, mb_id, it_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_status, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, ct_direct, ct_select, ct_select_time,top_seller )
                    VALUES ";
...
if ($sw_direct)
{
    ...
    //마케팅 중복 체크하고 입력하는 경우
    $check_overlap = $dao->checkMarketingLevelOverlap($it_id,$member["mb_id"]);
    if(!$check_overlap){
        //중복값 x
        //상태 1 (구매중) 으로 마케팅 데이터 insert
        $top_seller = $_POST["marketing_member"];
        $dao->setMarketingLevel($it_id,$top_seller,$member["mb_id"],"1");
    }
    if ($is_member)
    {
