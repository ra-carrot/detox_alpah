...
$it_id = $_POST["it_id"][0];
$mb_id = $member["mb_id"];
//무통장은 관리자에서 완료버튼 클릭하여 상태 변경할때만 마케팅 테이블 상태 값 변경
//if($od_settle_case != "무통장"){
//    $dao->setMarketingStatus($it_id, $mb_id, "1","4");
//    $dao->setMarketingExpenses($it_id,$mb_id,$od_id,$detox_ct_qty);
//}

//마케팅 레벨 상태 변경
$dao->setMarketingStatus($it_id, $mb_id, "1","4");
//마케팅 정보 없데이트 최초 구매 일 때만
$userInfo = $dao->getUserInfo($mb_id);
if($userInfo['marketing_members_first'] == null) $dao->setMarketingInfoProcess($it_id, $mb_id);
//마케팅 비용 update
$dao->setMarketingExpenses($it_id, $mb_id, $od_id,$detox_ct_qty);
